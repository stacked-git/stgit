...
test_expect_success 'Create a branch with empty patches' '
...
test_expect_success 'Attempt to sync patch not in ref branch' '
    stg new -m p4 &&
...
test_expect_success 'Synchronise the first two patches with the master branch' '
    stg sync -B master -a &&
    [ "$(echo $(stg series --applied --noprefix))" = "p1 p2 p3" ] &&
    [ "$(echo $(stg series --unapplied --noprefix))" = "" ] &&
...
test_expect_success 'Modify the master patches' '
    stg branch master &&
    [ "$(echo $(stg series --applied --noprefix))" = "p1 p2" ] &&
    [ "$(echo $(stg series --unapplied --noprefix))" = "p3" ] &&
    stg goto p1 &&
...
test_expect_success 'Synchronise the first two patches with the master branch (to fail)' '

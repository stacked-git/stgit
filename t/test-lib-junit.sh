...
start_test_output () {
	...
	junit_suite_start=$(fake_tool date getnanos)
...
start_test_case_output () {
	junit_start=$(fake_tool date getnanos)
}
...
finalize_test_case_output () {
	...
	failure)
		...
		junit_insert="$junit_insert $(xml_attr_encode \
			"$(if test -n "$GIT_TEST_TEE_OUTPUT_FILE"
			   then
				fake_tool path-utils skip-n-bytes \
	...
	junit_attrs="$junit_attrs time=\"$(fake_tool \
...
finalize_test_output () {
	...
	then
		...
		junit_time=$(fake_tool date getnanos $junit_suite_start)
...
xml_attr_encode () {
	...
	then
		shift
		printf '%s' "$*" | fake_tool xml-encode
	else
		printf '%s\n' "$@" | fake_tool xml-encode
	fi
}

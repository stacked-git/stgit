...
pub(super) fn command() -> clap::Command<'static> {
    clap::Command::new("send")
        ...
        .override_usage(
            "stg email send [OPTIONS] (-P <file|directory>|-p <patch>)... [-- <GIT-OPTIONS>]\n    \
        ...
        .arg(
            Arg::new("paths")
                .long("path")
                .short('P')
                .help("Patch files or directory of patch files to send")
                .long_help(
                    "Patches to send. May be patch files (as generated by `stg email \
                     format` or `git format-patch`) or directories (which will send all \
                     files in each directory), or patch names from the series.",
                )
                .value_name("path")
                .multiple_values(true)
                .value_parser(clap::value_parser!(PathBuf))
                .conflicts_with_all(&["paths-or-patchranges", "all"])
                .required_unless_present_any(&[
                    "patchranges",
                    "paths-or-patchranges",
                    "all",
                    "dump-aliases",
                ]),
        )
        .arg(
            Arg::new("patchranges")
                .long("patch")
                .short('p')
                .help("Patch names to send")
                .long_help(
                    "Patches to send. May be patch names from the series.\n\
                     \n\
        ...
        .arg(
            Arg::new("paths-or-patchranges")
                ...
                .multiple_values(true)
                .value_parser(clap::builder::NonEmptyStringValueParser::new())
                .conflicts_with_all(&["paths", "patchranges", "all"])
                .required_unless_present_any(&["paths", "patchranges", "all", "dump-aliases"]),
        )
        .arg(
...
fn administer_options() -> Vec<Arg<'static>> {
    vec![
        ...
        Arg::new("dump-aliases")
            ...
            .conflicts_with_all(&["paths", "patchranges", "paths-or-patchranges", "all"]),
    ]
}
...
pub(super) fn dispatch(matches: &clap::ArgMatches) -> Result<()> {
    ...
    let path_source_args = matches.get_many::<PathBuf>("paths");
    let patchrange_source_args = matches.get_many::<String>("patchranges");
    let path_or_patchrange_source_args = matches.get_many::<String>("paths-or-patchranges");
    let mut path_sources = Vec::new();
    let sources = if path_source_args.is_some()
        || patchrange_source_args.is_some()
        || path_or_patchrange_source_args.is_some()
    {
        let patchrange_source_args = {
            if let Some(ref paths_or_patchranges) = path_or_patchrange_source_args {
                let paths = paths_or_patchranges.clone().map(Path::new);
                if paths.clone().all(|p| p.is_dir() || p.is_file()) {
                    path_sources.extend(paths.map(OsString::from));
                    None
                } else {
                    path_or_patchrange_source_args
                }
            } else {
                path_sources.extend(
                    matches
                ...
                patchrange_source_args
            }
        };
        if let Some(patchranges) = patchrange_source_args {
            ...
            if last == stack.branch_head.id() {
                vec![OsString::from(format!("{base}..HEAD"))]
            } else {
                vec![OsString::from(format!("{base}..{last}"))]
            }
        } else {
            Vec::new()
        }
    ...
    let mut send_args = send_args
        .drain(..)
        .map(|(_, s)| OsString::from(s))
        .collect::<Vec<_>>();

    if let Some(options) = matches.get_many::<String>("git-send-email-opts") {
        send_args.extend(options.cloned().map(OsString::from));
    }

    let mut sources = sources;
    send_args.append(&mut sources);
    if !path_sources.is_empty() {
        send_args.push(OsString::from("--"));
        send_args.append(&mut path_sources);
    }
